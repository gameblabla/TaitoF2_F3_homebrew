//*****************************************************************
//* PCX to Taito F2 tile converter.
//*****************************************************************
...
{
	printf("PCX to Taito F2 tile converter.\n");
...
void Usage()
{
	printf("Command syntax is: TileF2 Offset Infile Rom [Palette]\n");
...
int Rip(char *filename)
{
    ...
    for (int yloop = 0; yloop < (Height / TILEDIMENSION); yloop++)
    {
        for (int xloop = 0; xloop < (Width / TILEDIMENSION); xloop++)
        {
            ...
            {
				for (int tilex = 0; tilex < TILEDIMENSION; tilex += 4)
                {
					BYTE Pixel1	= Tile[tiley * TILEDIMENSION + (TILEDIMENSION - 1) - 
						tilex] & 0x0f;
					BYTE Pixel2	= Tile[tiley * TILEDIMENSION + (TILEDIMENSION - 1) - 
						(tilex + 1)] & 0x0f;
					BYTE Pixel3	= Tile[tiley * TILEDIMENSION + (TILEDIMENSION - 1) - 
						(tilex + 2)] & 0x0f;
					BYTE Pixel4	= Tile[tiley * TILEDIMENSION + (TILEDIMENSION - 1) - 
						(tilex + 3)] & 0x0f;
				
					*Buffer		= (Pixel3 << 4) | Pixel4;
					Buffer++;
